...
Enable **Stripe Connect** on the platform account and finish the Connect marketplace / platform profile (including acknowledging negative-balance responsibilities). Organizer onboarding uses **Accounts v2** recipient accounts with Express Dashboard access; entry fees are destination charges to that account. Point a webhook at `POST /api/stripe/webhook` for `checkout.session.completed` (and optionally `checkout.session.async_payment_succeeded`). `APP_URL` is used for Checkout and Connect onboarding return URLs.
