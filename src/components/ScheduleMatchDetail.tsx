...
function MatchTeamPanel({
  ...
  return (
    <article className="ui-card min-w-0 overflow-hidden">
      <div className="border-b border-[var(--line)] bg-[linear-gradient(135deg,rgba(91,140,255,0.08),transparent_55%)] px-4 py-4">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <h4 className="mt-1 break-words font-[family-name:var(--font-display)] text-xl leading-tight tracking-tight text-[var(--ink)]">
              {teamName}
            </h4>
            {team ? (
            ...
            <ul className="ui-card divide-y divide-[var(--line)] overflow-hidden">
              {team.players.map((player) => (
                ...
                >
                  ...
                  <span className="shrink-0 tabular-nums font-semibold text-[var(--chalk)]">
                    {player.fargoRating}
                  </span>
                </li>
              ))}
...
export function ScheduleMatchDetail({
  ...
  return (
    <section className="animate-rise mx-auto w-full max-w-6xl space-y-5 md:space-y-6">
      <div className="min-w-0">
        <Button
          type="button"
          variant="secondary"
          onClick={onClose}
          className="mb-3 !rounded-full !px-3.5 !py-1.5 !text-xs md:mb-4"
        >
        ...
        <PageHeader
          ...
          description={match.location || undefined}
        />
      </div>
      ...
      <div className="space-y-3 md:hidden">
        <div
          ...
          className="grid grid-cols-2 gap-2 rounded-[var(--radius-sm)] border border-[var(--line)] bg-[color-mix(in_srgb,var(--surface-2)_80%,transparent)] p-1"
        >
          {sides.map((side) => {
